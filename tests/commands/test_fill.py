...
def test_fill_from_to():
    assert "fill ~ ~ ~ ~ ~ ~" == str(
        commands.fill.from_(positions.relative).to(positions.relative)
    )


def test_fill_from_to_block():
    assert "fill ~ ~ ~ ~ ~ ~ minecraft:grass" == str(
        commands.fill.from_(positions.relative)
        .to(positions.relative)
        .block(blocks.grass)
    )
...
def test_fill_call_replace_filter():
    assert "fill ~ ~ ~ ~ ~ ~ minecraft:grass replace minecraft:dirt" == str(
        commands.fill(
            positions.relative, positions.relative, blocks.grass
        ).replace.filter(blocks.dirt)
    )
