...
def test_give_targets_item():
    assert 'give @a minecraft:diamond' == str(commands.give.targets(selectors.all_players).item(items.diamond))
...
def test_give_call():
    assert 'give @a minecraft:diamond 64' == str(commands.give(selectors.all_players, items.diamond, 64))
