...
def test_execute_run_something_with_optional_args():
    assert "execute run clear @a" == str(
        commands.execute.run.clear.targets(selectors.all_players)
    )


def test_execute_run_something_nested():
    assert "execute run time query gametime" == str(
        commands.execute.run.time.query.gametime
    )
