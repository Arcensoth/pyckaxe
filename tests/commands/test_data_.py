...
def test_data_get_block_position():
    assert "data get block ~ ~ ~" == str(
        commands.data.get.block.position(positions.relative)
    )
...
def test_data_get_entity_target():
    assert "data get entity @r" == str(
        commands.data.get.entity.target(selectors.random)
    )
...
def test_data_merge_block_position():
    assert "data merge block ~ ~ ~" == str(
        commands.data.merge.block.position(positions.relative)
    )
...
def test_data_merge_entity_target():
    assert "data merge entity @r" == str(
        commands.data.merge.entity.target(selectors.random)
    )


def test_data_merge_entity_target_nbt():
    assert "data merge entity @r {Motion:[0.0f,0.5f,0.0f]}" == str(
        commands.data.merge.entity.target(selectors.random).nbt(
...
def test_data_remove_block_position():
    assert "data remove block ~ ~ ~" == str(
        commands.data.remove.block.position(positions.relative)
    )
...
def test_data_remove_entity_target():
    assert "data remove entity @r" == str(
        commands.data.remove.entity.target(selectors.random)
    )
