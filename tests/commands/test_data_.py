...
def test_data_merge_block_call():
    assert 'data merge block ~ ~ ~ {auto:1b}' == str(commands.data.merge.block(positions.relative, '{auto:1b}'))
...
def test_data_remove_block_position_path():
    assert 'data remove block ~ ~ ~ Item' == str(commands.data.remove.block.position(positions.relative).path('Item'))


def test_data_remove_block_call():
    assert 'data remove block ~ ~ ~ Item' == str(commands.data.remove.block(positions.relative, 'Item'))
...
def test_data_remove_entity_target_path():
    assert 'data remove entity @r Items' == str(commands.data.remove.entity.target(selectors.random).path('Items'))


def test_data_remove_entity_call():
    assert 'data remove entity @r Items' == str(commands.data.remove.entity(selectors.random, 'Items'))
