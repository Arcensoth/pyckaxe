...
def test_time_set():
    assert 'time set' == str(commands.time.set_)


def test_time_set_time():
    assert 'time set 9000' == str(commands.time.set_.time(9000))


def test_time_set_call():
    assert 'time set 9000' == str(commands.time.set_(9000))


def test_time_day():
    assert 'time set day' == str(commands.time.set_.day)


def test_time_midnight():
    assert 'time set midnight' == str(commands.time.set_.midnight)


def test_time_night():
    assert 'time set night' == str(commands.time.set_.night)


def test_time_noon():
    assert 'time set noon' == str(commands.time.set_.noon)
