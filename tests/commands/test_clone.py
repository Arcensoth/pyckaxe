...
def test_clone_begin_end():
    assert 'clone ~ ~ ~ ~ ~ ~' == str(commands.clone.begin(positions.relative).end(positions.relative))


def test_clone_begin_end_destination():
    assert 'clone ~ ~ ~ ~ ~ ~ ~ ~ ~' == str(
        commands.clone.begin(positions.relative).end(positions.relative).destination(positions.relative))
...
def test_clone_call_filtered_filter():
    assert 'clone ~ ~ ~ ~ ~ ~ ~ ~ ~ filtered minecraft:dirt' == str(
        commands.clone(positions.relative, positions.relative, positions.relative).filtered.filter(blocks.dirt))


def test_clone_call_filtered_call():
    assert 'clone ~ ~ ~ ~ ~ ~ ~ ~ ~ filtered minecraft:dirt' == str(
        commands.clone(positions.relative, positions.relative, positions.relative).filtered(blocks.dirt))


def test_clone_call_filtered_call_force():
    assert 'clone ~ ~ ~ ~ ~ ~ ~ ~ ~ filtered minecraft:dirt force' == str(
        commands.clone(positions.relative, positions.relative, positions.relative).filtered(blocks.dirt).force)


def test_clone_call_filtered_call_move():
    assert 'clone ~ ~ ~ ~ ~ ~ ~ ~ ~ filtered minecraft:dirt move' == str(
        commands.clone(positions.relative, positions.relative, positions.relative).filtered(blocks.dirt).move)


def test_clone_call_filtered_call_normal():
    assert 'clone ~ ~ ~ ~ ~ ~ ~ ~ ~ filtered minecraft:dirt normal' == str(
        commands.clone(positions.relative, positions.relative, positions.relative).filtered(blocks.dirt).normal)
