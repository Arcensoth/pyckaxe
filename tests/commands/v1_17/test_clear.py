...
def test_clear_targets_item():
    assert "clear @a minecraft:diamond" == str(
        commands.clear.targets(selectors.all_players).item(items.diamond)
    )
...
def test_clear_call():
    assert "clear @a minecraft:diamond 64" == str(
        commands.clear(selectors.all_players, items.diamond, 64)
    )
