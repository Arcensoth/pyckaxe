...
def test_tag_call_add_tag():
    assert "tag @a add sometag" == str(commands.tag(selectors.all_players).add.tag("sometag"))


def test_tag_call_add_call():
    assert "tag @a add sometag" == str(commands.tag(selectors.all_players).add("sometag"))
...
def test_tag_call_remove_tag():
    assert "tag @a remove sometag" == str(commands.tag(selectors.all_players).remove.tag("sometag"))


def test_tag_call_remove_call():
    assert "tag @a remove sometag" == str(commands.tag(selectors.all_players).remove("sometag"))
