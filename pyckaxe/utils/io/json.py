...
def load_json(path: Path, **options: Any) -> JsonValue:
...
def dump_json(data: JsonValue, path: Path, mkdir: bool = False, **options: Any):
...
async def load_json_async(path: Path, **options: Any) -> JsonValue:
...
async def dump_json_async(
    ...
    loop = asyncio.get_running_loop()
    await loop.run_in_executor(None, dump_json, data, path, mkdir, **options)
